...
public class BasicMapDemo {
	...
	private static void process(String path, int blockNum) throws IOException {
		...
		for (int i = 0; i < blockNum; i++) {
			BlockReader buffer = buffers.get(i);
			parseMap(buffer,map);
		}
		...
		for (int i = 0; i < map.table.length; i++) {
			...
			while (entry != null) {
				result.put(new String(entry.key, StandardCharsets.UTF_8).trim(), entry.data);
				entry = entry.next;
			}
		}
		...
		//System.out.println(" 冲突次数： " + map.conflictCounter);
	...
	private static void parseMap(BlockReader buffer, BasicMap map) {
